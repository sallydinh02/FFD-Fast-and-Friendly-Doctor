...
// Commit the transaction
    ...
    public static void doctorCreate(){
        ...
        for(int i=0;i<doctor.length;i++) {
            ...
            user.put("DateOfBirth", dateToString(doctor[i].DateOfBrith,"MM/dd/yyyy"));
        ...
        for(int i=0;i<patient.length;i++) {
            ...
            user.put("DateOfBirth", dateToString(patient[i].DateOfBrith,"MM/dd/yyyy"));
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if(task.isSuccessful()){
                            for(QueryDocumentSnapshot document: task.getResult()){
                                ...
                                d.DateOfBrith= FirebaseNumberAndDateTimeProcess.stringToDate(u.get("DateOfBirth").toString(),"MM/dd/yyyy");
                                doctorList.add(d);
                            }
                        }
                    }
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if(task.isSuccessful()){
                            for(QueryDocumentSnapshot document: task.getResult()){
                                ...
                                p.DateOfBrith= FirebaseNumberAndDateTimeProcess.stringToDate(u.get("DateOfBirth").toString(),"MM/dd/yyyy");
                                patientList.add(p);
                            }
                        }
                    }
